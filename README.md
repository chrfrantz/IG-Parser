...
1. In Google Sheets, mark/select the relevant section (or entire table) using `Ctrl + A`, or by clicking on the top left area (`Select all` area  above the line number and left to the columns),
2. Copy the highlighted content (`Ctrl + C`), and
3. Paste it into the same area as "values only" (by rightclicking and selecting "Paste special", and the option "Values only" in the sub menu.
