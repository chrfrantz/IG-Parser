...
When using Google Sheets output generated by the IG Parser and pasting this into Google Sheets, by default the generated spreadsheet output contains some formulae in all cells, including (at first sight) empty cells (e.g., `=IFERROR(@__xludf.DUMMYFUNCTION("""COMPUTED_VALUE""");" ")` for empty cells). 

To clean the spreadsheet (i.e., remove formulae and ensure that empty cells are actually empty), do the following in Google Sheets (before exporting):
 
1. In Google Sheets, mark/select the relevant section (or entire table) using `Ctrl + A`, or by clicking on the top left area (referred to as "'Select all' area"  above the line number and left to the column letters),
2. Copy the highlighted content (`Ctrl + C`, or by rightclicking and selecting "Copy"), and
3. Paste it into the same area as "Values only" (`Ctrl + Shift + V`, or by rightclicking and selecting "Paste special", and the option "Values only" in the sub menu).

This will overwrite all cell content with the *actually displayed values*, and removes the underlying formulae. However, this *does still not remove invisible whitespace values* from seeminglly empty cells. To remove those, do the following:

4. Open the "Find and Replace" dialog (`Ctrl + F`), and click on the three vertical buttons (they have the tooltip `More options`) that open the full dialog.
5. In that opening dialog, select the option "Match entire cell contents", and add a whitespace into the "Find" text field (without quotations marks, just a whitespace). Leave the "Replace with" field empty and click "Replace all". This should remove all entries from empty cells (and in fact tell you how many whitespaces have been replaced). Click on "Done" to leave the dialog. *Important:* Don't forget to select the "Match entire cell contents", else all whitespaces (also the ones contained in text) will be removed!
6. The sheet should now be clean (i.e., only contain the data and every cell that looks empty should actually be empty). You can now work with the data, or export it to Excel or any other format you would like to use for further processing (via Menu `File`, `Download`).

Please note that this aspect is specific to Google Sheets output; it does not apply to CSV output. 
...
Annotations can be used with all sub-patterns occurring in institutional statements, including component-level nesting, nested component combinations, as well as component-pair combinations and will be handled as follows:

* *Component-Level Nesting*: In addition to the annotation on the nesting component (`[outer annotation]` in the following example `Cac[outer annotation]{ A(actor) ... [inner annotation] ... }`, annotations embedded in the nested parts (`[inner annotation`) are also supported, and concatenated with potentially existing annotations on the nesting component (`[outer annotation`) as part of the output generation. For this example, the statement-level annotation for this nested component would be `[outer annotation][inner annotation]`.

* *Nested Component Combinations*: Similar to component-level nesting, annotations on outer component and statement branches are considered as statement-level annotation. For example, `O[orElseAnnotation]{{ [left annotation] A(actor1) I(act1) [AND] [right annotation] A(actor2) I(act2) }}` has a nested component annotation (`[orElseAnnotation]`) and annotations specific to branches concatenated for the respective row in the output. As part of the output generation, those are treated as  `[orElseAnnotation][left annotation]` for the left side, and `[orElseAnnotation][right annotation]` for the right side.

* *Component-Pair Combinations*: Combinations consisting of multiple components on either side ("component pairs") are handled accordingly. For example, for the expression `{ [left annotation] A(leftActor) I(leftAct) [XOR] [right annotation] A(rightActor) [another annotation] I(rightAct) }`, the `[left annotation]` would be associated with the left-side component pair (i.e., `A(leftActor) I(leftAct)`). The annotations `[right annotation]` and `[another annotation]` would be associated with the right-side pair, and concatenated as `[right annotation][another annotation]` in the output.
...
* Semantic annotation on component pair combination within nested component (output concatenates leading annotation with branch-specific ones):
 ...
 ### Common issues
  * Parentheses/braces/brackets need to match. The parser calls out if a mismatch exists. Parentheses are applied when components are specified (or combinations thereof), braces are used to signal component-level nesting (i.e., statements embedded within components) and statement combinations (i.e., combinations of component-level nested statements), and brackets are used for annotations.
    * Special Case 1 *Original statement input contains parentheses as means to structure text*: The input itself (i.e., the content to be encoded) may contain parentheses (e.g., `... under the following conditions 1) the actor must not ..., 2) the actor must not ...`). In such cases, the parentheses must be removed as part of the preprocessing of the text input, since the parser cannot differentiate between parentheses used for the encoding and the ones contained in the content.
    * Special Case 2 *Nested brackets*: Use of brackets *within* annotations (e.g., `[outer[inner]annotation]`) will lead to the extraction of the inner annotations (i.e., `[inner]`) only. The remaining parts (concatenated as `[outerannotation]`) will be ignored during parsing. However, the parser should provide an associated warning as part of the output generation. Parentheses within annotations (e.g., `[function(value1, value2)]`) are tolerated by the parser and included in the generated output.
