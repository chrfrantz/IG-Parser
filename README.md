...
This will overwrite all cell content with the *actually displayed values*, and removes the underlying formulae. Remember to save the result. Please note that this aspect is specific to Google Sheets output; it does not apply to CSV output. As an alternative strategy (e.g., where copying and pasting is not an option), you can simply replace all occurrences of the function (see above) with empty/blank entries using the "Find and Replace" function in Excel or other spreadsheet software.
