...
/*
Tests substitution of special symbols (quotation marks) during output generation for IG Core and annotations.
*/
func TestTabularOutputSubstitutionSpecialSymbolsDuringOutputIGCore(t *testing.T) {

	// Statement with quotation marks all over the place
	originalStatement := "\"Test statement with random \"\"quotation\"\" marks every\"where"

	// Statement with multi-level nesting with embedded nested statement combinations as well as annotations, with quotation marks sprinkled across (to test robust substitution)
	text := "A[quot=\"annotation\"](actor0) I(\"aim0\") Cac[\"directAnnotation\"]{ Cac{ A(actor6) I(aim\"6) } [XOR] Cac{ A(actor7) I(actor7) }} Cac{ Cac{ A(\"actor1\") I(aim1) } [OR] Cac{ A(actor0) I(aim0) Cac{ A(actor2) I(aim2) Cac{ Cac{ A(actor3) I(aim3) } [OR] Cac{ A(actor4) I[\"anotherAnnotation\"](aim4) } } Bdir(object2) } [OR] Cac{ A(actor5) I(aim5)} }}"
	...
	// Read reference file
	content, err2 := os.ReadFile("TestTabularOutputSubstitutionSpecialSymbolsDuringOutputIGCore.test")
	if err2 != nil {
		t.Fatal("Error attempting to read test text input. Error: ", err2.Error())
	}
...
/*
Tests substitution of special symbols (quotation marks) during output generation for IG Extended and annotations for Google Sheets output.
*/
func TestTabularOutputSubstitutionSpecialSymbolsDuringOutputIGExtendedGoogleSheets(t *testing.T) {
	...
	// Read reference file
	content, err2 := os.ReadFile("TestTabularOutputSubstitutionSpecialSymbolsDuringOutputIGExtendedGoogleSheets.test")
	if err2 != nil {
		t.Fatal("Error attempting to read test text input. Error: ", err2.Error())
	}
...
/*
Tests substitution of special symbols (quotation marks) during output generation for IG Extended and annotations for CSV output.
*/
func TestTabularOutputSubstitutionSpecialSymbolsDuringOutputIGExtendedCSV(t *testing.T) {
	...
	// Read reference file
	content, err2 := os.ReadFile("TestTabularOutputSubstitutionSpecialSymbolsDuringOutputIGExtendedCSV.test")
	if err2 != nil {
		t.Fatal("Error attempting to read test text input. Error: ", err2.Error())
	}
...
/*
Tests proper handling of all kinds of diacritics across nesting levels as well as in component content and annotations.
*/
func TestTabularOutputDiacriticsHandling(t *testing.T) {

	// Statement with quotation marks all over the place
	originalStatement := "Test stßatemøÅæent with randÈ, Ì, om diacriäöütics everïiywhere"

	// Statement with multi-level nesting with embedded nested statement combinations as well as annotations, with quotation marks sprinkled across (to test robust substitution)
	text := "A,p(Regional) A[role=enforcer,type=animate](Manaægers), Cex(on behalf of the Secretary), D[stringency=permissive](may) I[act=perfà, è, ì, ò, ù, À, È, Ì, Òormance](review [AND] (reward [XOR] sanction)) Bdir,p(approved) Bdir1,p(certified) Bdir1[role=monitored,type=aniömate](producütion [opeärations]) and Bdir[role=monžöjřitored,type=animate](handling operations) and Bdir2,p(accrediçted) Bdir2[role=monitor,type=animate](certifying agents) ïtext Cex[ctx=purpose](for compliance with the (Act [XOR] regulations in this part)) under the condition that Cac{Cac[state]{A[role=monitored,type=animate](Operations) I[act=violate](wereÅ non-compliant [OR] violated) Bdir[type=inò, ù, À, È, Ì, Ònimate](organic farming provisions)} [AND] Cac[state]{A[role=enforcer,type=animate](Manaò, ù, À, È, Ì, Òer) I[act=terminate](has øconcluded) Bdir[type=activity](investigation)}}."
	...
	// Read reference file
	content, err2 := os.ReadFile("TestTabularOutputDiacriticsHandling.test")
	if err2 != nil {
		t.Fatal("Error attempting to read test text input. Error: ", err2.Error())
	}
...
Tests substitution of special symbols (quotation marks) during output generation for IG Core and annotations.
*/
func TestTabularOutputStatementLevelAnnotations(t *testing.T) {

	// Statement with quotation marks all over the place
	originalStatement := "does not really matter for this test"

	// Statement with statement-level annotations, annotated conditions, as well as component-level embedded brackets
	text := "Cac[conditionLevelAnnotation]{Once E(policy) [internalAnnotation] F(comes into force)} A,p(relevant) [annotation1] A(regulators) D([must]) I(monitor [AND] enforce) Bdir(compliance). [annotation2]"
	...
	// Read reference file
	content, err2 := os.ReadFile("TestTabularOutputStatementLevelAnnotations.test")
	if err2 != nil {
		t.Fatal("Error attempting to read test text input. Error: ", err2.Error())
	}
