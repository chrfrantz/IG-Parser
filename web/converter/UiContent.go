...
var ANNOTATED_STATEMENT = "A,p(Regional) A(Managers), Cex(on behalf of the Secretary), D[permissive](may) I[act=performance]((review [AND] (reward [XOR] sanction))) Bdir,p(approved) Bdir,p1(certified) Bdir1(production [operations]) and Bdir(handling operations) and Bdir,p2(accredited) Bdir2(certifying agents) Cex(for compliance with the (Act or [XOR] regulations in this part)) under the condition that Cac{A(Operations) I[violate](were (non-compliant [OR] violated)) Bdir(organic farming provisions)}."
// Default example ID
